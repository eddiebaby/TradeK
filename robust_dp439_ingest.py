#!/usr/bin/env python3
"""
Robust ingestion for dp439.pdf using pdftotext
"""
...
def extract_with_pdftotext(pdf_path):
    ...
    print("📖 Extracting PDF content with pdftotext...")
...
def create_intelligent_chunks(text, chunk_size=1000):
...
def save_to_sqlite(book_id, title, author, chunks, file_path):
...
def test_semantic_search(book_id, title):
    """Test semantic search on the ingested paper"""
    ...
    try:
        ...
        test_queries = [
            ...
            "quantile estimation return distribution",
            "volatility scaling financial markets"
        ]
...
def main():
    ...
    # Extract content
    content = extract_with_pdftotext(pdf_path)
    if not content:
        print("❌ No content extracted!")
        return
    
    # Create chunks
    chunks = create_intelligent_chunks(content, chunk_size=1200)
    if not chunks:
    ...
    author = "Jón Daníelsson, Jean-Pierre Zigrand"
    ...
    # Save to databases
    sqlite_success = save_to_sqlite(book_id, title, author, chunks, pdf_path)
