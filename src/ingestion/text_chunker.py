...
class TextChunker:
    ...
    def __init__(self, config: ChunkingConfig = None):
        ...
        self.formula_pattern = re.compile(r"\$\$[\s\S]*?\$\$|\$[^\$]+\$", re.MULTILINE)

    def chunk_text(
        self, text: str, book_id: str, metadata: dict[str, Any] = None
    ) -> list[Chunk]:
        """
        ...
        chunk_objects = self._create_chunk_objects(chunks, book_id, metadata or {})
    ...
    def chunk_pages(
        self, pages: list[dict[str, Any]], book_id: str, metadata: dict[str, Any] = None
    ) -> list[Chunk]:
        """
        ...
        for page in pages:
            ...
            if len(accumulated_text) >= self.config.chunk_size:
                ...
                if chunks and self.config.chunk_overlap > 0:
                    ...
                    overlap_start = max(
                        0, len(last_chunk_text) - self.config.chunk_overlap
                    )
    ...
    def _create_chunks(
        self, text: str, special_regions: list[tuple[int, int, str]]
    ) -> list[str]:
        """
    ...
    def _find_break_point(self, text: str, start: int, ideal_end: int) -> int:
        ...
        if self.config.respect_sentences:
            sentence_breaks = list(
                self.sentence_end_pattern.finditer(text[start : ideal_end + 50])
            )
            if sentence_breaks:
    ...
    def _create_chunk_objects(
        self, text_chunks: list[str], book_id: str, metadata: dict[str, Any]
    ) -> list[Chunk]:
        """
...
def test_chunker():
    ...
    config = ChunkingConfig(chunk_size=300, chunk_overlap=50, preserve_code_blocks=True)
