...
class PDFParser:
    ...
    def parse_file(self, file_path: Path) -> dict[str, Any]:
        ...
        try:
            ...
            if loop.is_running():
                ...
                raise RuntimeError(
                    "Cannot call parse_file() from async context. Use parse_file_async() instead."
                )
            else:
    ...
    async def parse_file_async(self, file_path: Path) -> dict[str, Any]:
        ...
        try:
            ...
            metadata, pages = await asyncio.to_thread(
                self._parse_with_pypdf2, file_path
            )
            ...
            if self._is_extraction_poor(pages):
                ...
                metadata_plumber, pages_plumber = await asyncio.to_thread(
                    self._parse_with_pdfplumber, file_path
                )
            ...
            if (
                self._is_extraction_poor(result["pages"])
                and self.enable_ocr
                and self._ocr_processor
            ):
                logger.info(
                    "Poor text extraction detected, checking if OCR is needed..."
                )
                ...
                if needs_ocr:
                    ...
                    if ocr_pages and self._count_words(ocr_pages) > self._count_words(
                        result["pages"]
                    ):
                        ...
                        result["metadata"]["ocr_confidence"] = (
                            sum(p.get("confidence", 0) for p in ocr_pages)
                            / len(ocr_pages)
                            if ocr_pages
                            else 0.0
                        )
                        logger.info(
                            f"OCR completed with average confidence: {result['metadata']['ocr_confidence']:.1f}%"
                        )
            ...
            with open(file_path, "rb") as file:
                ...
                if reader.metadata:
                    metadata = {
                        ...
                        "producer": self._clean_text(
                            reader.metadata.get("/Producer", "")
                        ),
                ...
                logger.info(
                    f"Processing {total_pages} pages in batches of {BATCH_SIZE}"
                )
    ...
    def _parse_with_pdfplumber(
        ...
        recommendations = resource_monitor.get_processing_recommendations()
        BATCH_SIZE = max(
            5, recommendations["pdf_batch_size"] // 2
        )  # Smaller for pdfplumber
        ...
        try:
            with pdfplumber.open(file_path) as pdf:
                ...
                logger.info(
                    f"Processing {total_pages} pages with pdfplumber in batches of {BATCH_SIZE}"
                )
                ...
                for batch_start in range(0, total_pages, BATCH_SIZE):
                    batch_end = min(batch_start + BATCH_SIZE, total_pages)
                    logger.debug(
                        f"Processing pages {batch_start + 1}-{batch_end} with pdfplumber"
                    )
                    ...
                    for page_num in range(batch_start, batch_end):
                        try:
                            ...
                            if (
                                len(text.strip()) < 100
                            ):  # Only extract tables if text is sparse
                                try:
                            ...
                            logger.warning(
                                f"Error extracting page {page_num + 1} with pdfplumber: {e}"
                            )
                            pages.append(
                                {
                                    "page_number": page_num + 1,
                    ...
                    if len(pages) % 25 == 0:  # Log progress every 25 pages
                        logger.info(
                            f"Processed {len(pages)}/{total_pages} pages with pdfplumber"
                        )
    ...
    def _post_process_results(
        ...
        result["statistics"] = {
            ...
            "average_words_per_page": (
                total_words / len(result["pages"]) if result["pages"] else 0
            ),
        }
...
def test_parser():
    ...
    if test_file.exists():
        ...
        if result["pages"]:
            ...
            sample = (
                first_page["text"][:200] + "..."
                if len(first_page["text"]) > 200
                else first_page["text"]
            )
            print(f"\nFirst page sample:\n{sample}")
    else:
