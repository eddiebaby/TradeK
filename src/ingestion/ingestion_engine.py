...
class IngestionEngine:
    ...
    async def get_ingestion_status(self) -> dict[str, Any] | None:
    ...
    async def _parse_file(self, file_path: Path) -> dict[str, Any]:
        ...
        if file_path.suffix.lower() == ".pdf":
            ...
            return await loop.run_in_executor(
                self.process_pool, self.pdf_parser.parse_file, file_path
            )
        else:
    ...
    async def _create_book_record(
        ...
        book = Book(
            ...
            file_type=(
                FileType.PDF if file_path.suffix.lower() == ".pdf" else FileType.EPUB
            ),
            file_hash=file_hash,
    ...
    async def _chunk_book(
        self, pages: list[dict[str, Any]], book_id: str
    ) -> list[Chunk]:
        """Chunk book pages"""
        # Update status
        if self.current_status:
        ...
        chunks = await asyncio.to_thread(
            self.text_chunker.chunk_pages, pages, book_id, {}
        )
