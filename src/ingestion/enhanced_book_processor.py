...
class EnhancedBookProcessor:
    ...
    def __init__(self):
        ...
        # Storage
        self.embedding_generator: LocalEmbeddingGenerator | None = None
        self.sqlite_storage: SQLiteStorage | None = None
        self.vector_storage: QdrantStorage | None = None

        # Phase 2 components
        self.cache_manager = None
        self.query_suggester: QuerySuggester | None = None

        # Processing state
        self.current_status: IngestionStatus | None = None

        # Resource monitoring
        self.resource_monitor = get_resource_monitor()

        # Supported file types
        self.supported_extensions = {".pdf": FileType.PDF, ".epub": FileType.EPUB}
    ...
    async def add_book(
        ...
        try:
            ...
            result = {
                ...
                "ocr_used": parse_result.get("metadata", {}).get(
                    "ocr_processed", False
                ),
            ...
            try:
                ...
                if summary.get("peak_memory_exceeded"):
                    logger.warning(
                        "Peak memory usage exceeded limits during processing"
                    )
    ...
    async def remove_book(self, book_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": f"Failed to remove book: {str(e)}"}

    async def list_books(self, category: str | None = None) -> list[dict[str, Any]]:
        ...
        for book in books:
            book_info = {
                ...
                "has_code": book.metadata.get("content_analysis", {}).get(
                    "has_code", False
                ),
                "has_formulas": book.metadata.get("content_analysis", {}).get(
                    "has_formulas", False
                ),
    ...
    async def get_book_details(self, book_id: str) -> dict[str, Any] | None:
        ...
        result = {
            ...
            "created_at": book.created_at.isoformat(),
            "indexed_at": book.indexed_at.isoformat() if book.indexed_at else None,
            "chunk_statistics": {"total": len(chunks), "by_type": chunk_types},
    ...
    async def _create_enhanced_book_record(
        self,
        file_path: Path,
        file_hash: str,
    ...
    async def _chunk_book_enhanced(
        self,
    ...
    async def _generate_embeddings_cached(
        ...
        if uncached_chunks:
            logger.info(
                f"Generating embeddings for {len(uncached_chunks)} uncached chunks..."
            )
            ...
            logger.info(
                f"Using dynamic batch size: {EMBEDDING_BATCH_SIZE} (memory status: {recommendations['memory_status']})"
            )

            for batch_start in range(0, len(uncached_chunks), EMBEDDING_BATCH_SIZE):
                batch_end = min(
                    batch_start + EMBEDDING_BATCH_SIZE, len(uncached_chunks)
                )
                batch_chunks = uncached_chunks[batch_start:batch_end]

                logger.debug(
                    f"Processing embedding batch {batch_start + 1}-{batch_end}"
                )

                # Generate embeddings for this batch
                batch_embeddings = await self.embedding_generator.generate_embeddings(
                    batch_chunks
                )

                # Cache and store results
                for j, (chunk, embedding) in enumerate(
                    zip(batch_chunks, batch_embeddings, strict=False)
                ):
                    ...
                    # Cache for 7 days
                    cache_key = (
                        f"embedding:{hashlib.md5(chunk.text.encode()).hexdigest()}"
                    )
                    await self.cache_manager.set(
                        cache_key, embedding, "embedding", ttl=604800
                    )
                ...
                if check["memory_critical"]:
                    logger.warning(
                        "Memory critical during embedding generation - waiting..."
                    )
                    await self.resource_monitor.wait_for_memory_available()

                if batch_end % 100 == 0:  # Log progress every 100 embeddings
                    logger.info(
                        f"Generated {batch_end}/{len(uncached_chunks)} embeddings (Memory: {self.resource_monitor.current_memory_percent:.1f}%)"
                    )

        logger.info(
            f"Embedding generation complete. Cache hit rate: {(len(chunks) - len(uncached_chunks)) / len(chunks) * 100:.1f}%"
        )
        return embeddings

    async def _update_search_suggestions(
        self, book: Book, content_analysis: dict[str, Any]
    ):
        ...
        if content_analysis.get("formulas"):
            potential_queries.extend(
                ["mathematical formulas", "trading formulas", "calculations"]
            )

        if content_analysis.get("strategies"):
            potential_queries.extend(
                ["trading strategies", "investment strategies", "algorithmic trading"]
            )
...
async def test_enhanced_processor():
    ...
    test_files = ["data/books/sample.pdf", "data/books/sample.epub"]
