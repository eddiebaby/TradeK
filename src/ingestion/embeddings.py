...
class EmbeddingGenerator:
    ...
    def __init__(self, config=None, model_name: str | None = None):
        """
    ...
    def _init_sentence_transformer(self):
        ...
        try:
            ...
            logger.error(
                "sentence-transformers not installed. Attempting OpenAI fallback."
            )
            try:
                self._init_openai()
            except ValueError as e:
                raise RuntimeError(
                    f"Neither local embeddings nor OpenAI available: {e}"
                ) from e
    ...
    async def generate_embeddings(
        ...
        if uncached_chunks:
            ...
            # Add to cache
            for chunk, embedding in zip(uncached_chunks, new_embeddings, strict=False):
                cache_key = self._get_cache_key(chunk.text)
...
async def test_embedding_generator():
    ...
    try:
        ...
        for i, (chunk, embedding) in enumerate(zip(test_chunks, embeddings, strict=False)):
