...
class NotebookParser:
    ...
    def __init__(self):
        ...
        self.patterns = {
            ...
            "analysis": re.compile(
                r"analysis|performance|metrics|sharpe|returns", re.I
            ),
    ...
    def can_parse(self, file_path: Path) -> bool:
        ...
        return (
            file_path.suffix.lower() in self.supported_extensions and NBFORMAT_AVAILABLE
        )
    ...
    def parse_file(self, file_path: Path) -> dict[str, Any]:
        ...
        try:
            ...
            markdown_cells = sum(
                1 for p in result["pages"] if p["cell_type"] == "markdown"
            )
    ...
    def _extract_metadata(self, notebook: Any, file_path: Path) -> dict[str, Any]:
        ...
        if hasattr(notebook, "metadata") and notebook.metadata:
            ...
            if "kernelspec" in nb_meta:
                metadata["kernel"] = nb_meta["kernelspec"].get(
...
def test_notebook_parser():
    ...
    if test_file.exists():
        ...
        if result["pages"]:
            ...
            sample = (
                first_cell["text"][:200] + "..."
                if len(first_cell["text"]) > 200
                else first_cell["text"]
            )
