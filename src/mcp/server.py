...
class TradeKnowledgeServer:
    ...
    def _register_tools(self):
        ...
        async def get_chunk_context(
            chunk_id: str, before_chunks: int = 2, after_chunks: int = 2
        ) -> dict[str, Any]:
            """
        ...
        @self.server.tool(
            name="list_books", description="List all books in the knowledge base"
        )
        async def list_books(category: str | None = None) -> dict[str, Any]:
            """
        ...
        @self.server.tool(
            name="add_book", description="Add a new book to the knowledge base"
        )
        async def add_book(
            file_path: str,
...
async def test_server():
    ...
    try:
        search_result = await server.server.tools[0]["func"](
            "trading strategies", "hybrid", 3
        )
        print(
            f"Search test result: {search_result.get('total_results', 0)} results found"
        )
    except Exception as e:
