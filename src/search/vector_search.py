...
class VectorSearchEngine:
    ...
    def __init__(
        ...
        self.collection_name = (
            collection_name or self.config.database.chroma.collection_name
        )
        self.persist_directory = (
            persist_directory or self.config.database.chroma.persist_directory
        )
    ...
    def _initialize_client(self):
        ...
        try:
            ...
            logger.info(
                f"Vector search engine initialized with {self.collection.count()} documents"
            )
    ...
    async def get_similar_chunks(
        self, chunk_id: str, num_results: int = 5
    ) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if sample_size > 0:
                ...
                return {
                    ...
                    "embedding_dimension": (
                        len(sample["embeddings"][0]) if sample.get("embeddings") else 0
                    ),
    ...
    def delete_book(self, book_id: str) -> bool:
        ...
        try:
            ...
            results = self.collection.get(where={"book_id": book_id}, include=["ids"])
...
async def search_vectors(
    query: str, num_results: int = 10, filter_books: list[str] | None = None
) -> dict[str, Any]:
