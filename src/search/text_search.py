...
class TextSearchEngine:
    ...
    def _sanitize_search_query(self, query: str) -> str:
            ...
            if pattern in query_lower:
                ...
                if pattern in ["select", "delete", "create"] and '"' in query:
                    ...
                    quoted_pattern = re.compile(
                        r'"[^"]*' + re.escape(pattern) + r'[^"]*"', re.IGNORECASE
                    )
        ...
        special_char_count = sum(
            1 for c in query if not (c.isalnum() or c.isspace() or c in '"*-_()[]')
        )
    ...
    def get_chunk_context(
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                """,
                    (book_id, chunk_index - before_chunks, chunk_index + after_chunks),
                )
                ...
                target_chunk = {
                    ...
                    "metadata": (
                        json.loads(target_row["metadata"])
                        if target_row["metadata"]
                        else {}
                    ),
                }
    ...
    def get_database_stats(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {"total_chunks": 0, "error": str(e), "database_path": self.db_path}
