...
from ..core.models import Chunk, SearchResponse, SearchResult
...
class HybridSearch:
    ...
    async def search_exact(
        ...
        try:
            ...
            results = await self.sqlite_storage.search_exact(
                query=query, book_ids=filter_books, limit=num_results
            )
    ...
    async def search_hybrid(
        ...
        try:
            ...
            search_multiplier = min(
                2, max(1.5, num_results / 5)
            )  # Scale based on requested results
            ...
            merged_results = self._merge_results(
                semantic_response["results"], exact_response["results"], semantic_weight
            )
            ...
            response = SearchResponse(
                ...
                search_type="hybrid",
                filters_applied=(
                    {"book_ids": filter_books, "semantic_weight": semantic_weight}
                    if filter_books
                    else {"semantic_weight": semantic_weight}
                ),
            )
    ...
    async def get_chunk_context(
        ...
        try:
            context = await self.sqlite_storage.get_chunk_context(
                chunk_id=chunk_id, before=before_chunks, after=after_chunks
            )
    ...
    def _merge_results(
        ...
        for chunk_id, data in result_map.items():
            ...
            combined_score = (
                semantic_score * semantic_weight + exact_score * exact_weight
            )
        ...
        sorted_results = sorted(
            result_map.values(), key=lambda x: x["result"]["score"], reverse=True
        )
                ...
                if isinstance(result_dict, dict):
                    ...
                    if isinstance(chunk_data, dict):
                        ...
                        if all(field in chunk_data for field in required_fields):
                            ...
                            logger.warning(
                                f"Chunk data missing required fields: {chunk_data}"
                            )
                            continue
                    ...
                    search_result = SearchResult(
                        ...
                        highlights=(
                            result_dict.get("highlights", [])
                            if isinstance(result_dict.get("highlights"), list)
                            else []
                        ),
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        avg_time = (
            self.total_search_time / self.search_count if self.search_count > 0 else 0
        )
...
async def test_search_engine():
    ...
    for query in test_queries:
        ...
        print(
            f"Found {results['total_results']} results in {results['search_time_ms']}ms"
        )
