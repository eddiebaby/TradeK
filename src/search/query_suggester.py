...
class QuerySuggester:
                        ...
                        suggestions.append(
                            {
                                ...
                                "type": "template",
                                "score": 0.6,
                                "metadata": {"template": template_name, "topic": topic},
                            }
                        )
    ...
    def _rank_suggestions(
        self, suggestions: list[dict[str, Any]], original_query: str
    ) -> list[dict[str, Any]]:
    ...
    async def _save_search_history(self):
        ...
        try:
            ...
            await self.cache_manager.set(
                "search_history", top_queries, "general", ttl=86400 * 7
            )  # 1 week
        except Exception as e:
            logger.error(f"Failed to save search history: {e}")
...
async def test_query_suggester():
    ...
    # Test suggestions
    test_queries = ["mov", "rsi", "python", "what is", "how to", "risk"]
    ...
    for query in test_queries:
        ...
        for i, suggestion in enumerate(suggestions, 1):
            print(
                f"  {i}. {suggestion['text']} ({suggestion['type']}, score: {suggestion['final_score']:.2f})"
            )
