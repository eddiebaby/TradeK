...
async def get_user(user_id: str, admin_user=Depends(require_admin)):
    """
    ...
    try:
        ...
        logger.info(
            "User details retrieved", admin_id=admin_user.id, target_user_id=user_id
        )
        return user
...
async def create_user(user_data: CreateUserRequest, admin_user=Depends(require_admin)):
    """
...
async def update_user(
    user_id: str, user_data: CreateUserRequest, admin_user=Depends(require_admin)
):
    """
...
async def update_system_config(
    config: SystemConfigUpdate, admin_user=Depends(require_admin)
):
    """
    ...
    try:
        ...
        logger.info(
            "System config updated", admin_id=admin_user.id, config=config.dict()
        )
...
@router.get("/config")
async def get_system_config(admin_user=Depends(require_admin)):
    """
...
async def cleanup_system(
    background_tasks: BackgroundTasks, admin_user=Depends(require_admin)
):
    """
