...
from ..main import get_current_user, get_metrics, get_search_engine
from ..models import *
    ...
    try:
        ...
        response = SearchResponse(
            ...
            intent=(
                ...
                if request.intent
                else search_result.get("detected_intent")
            ),
            results=results,
...
async def autocomplete_query(
    ...
    try:
        suggestions = await search_engine.get_suggestions(
            partial_query=q, max_suggestions=max_suggestions, user_id=user.id
        )
...
async def get_chunk_context(
    ...
    context_size: int = Query(
        2, ge=1, le=5, description="Number of chunks before/after"
    ),
    search_engine: UnifiedSearchEngine = Depends(get_search_engine),
    user=Depends(get_current_user),
):
    ...
    try:
        context_result = await search_engine.get_chunk_context(
            chunk_id=chunk_id, context_size=context_size, user_id=user.id
        )
...
async def find_similar_content(
    ...
    try:
        similar_results = await search_engine.find_similar(
            result_id=result_id, max_results=max_results, user_id=user.id
        )
...
async def get_trending_queries(
    ...
    """
    try:
        trending = await search_engine.get_trending_queries(period=period, limit=limit)
        ...
        raise HTTPException(
            status_code=500, detail=f"Trending queries failed: {str(e)}"
        )
...
async def export_search_results(
    ...
    try:
        # Perform search
        search_result = await search_engine.search(
            query=query, max_results=max_results, user_id=user.id
        )
