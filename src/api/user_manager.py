...
class UserManager:
    ...
    def __init__(self, db_path: str | None = None):
        ...
        self.db_path = db_path or str(
            Path(self.config.database.sqlite.path).parent / "users.db"
        )
    ...
    async def create_user(self, user_data: UserCreate) -> User:
        ...
        if len(user_data.password) < self.config.api.auth.min_password_length:
            raise ValueError(
                f"Password must be at least {self.config.api.auth.min_password_length} characters"
            )
                ...
                if self._verify_password(password, password_hash):
                    ...
                    if new_attempts >= self.config.api.auth.max_login_attempts:
                        lockout_time = datetime.utcnow() + timedelta(
                            minutes=self.config.api.auth.lockout_duration_minutes
                        )
