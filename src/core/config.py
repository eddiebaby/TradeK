...
class QdrantConfig(BaseModel):
    ...
    port: int = Field(
        default_factory=lambda: _safe_int(os.getenv("QDRANT_PORT", "6333"), 6333)
    )
    collection_name: str = Field(
        default_factory=lambda: os.getenv("QDRANT_COLLECTION", "tradeknowledge")
    )
    use_grpc: bool = Field(
        default_factory=lambda: os.getenv("QDRANT_USE_GRPC", "false").lower() == "true"
    )
    ...
    prefer_grpc: bool = Field(
        default_factory=lambda: os.getenv("QDRANT_PREFER_GRPC", "false").lower()
        == "true"
    )
...
class EmbeddingConfig(BaseModel):
    ...
    model: str = Field(
        default_factory=lambda: os.getenv("OLLAMA_MODEL", "nomic-embed-text")
    )
    dimension: int = Field(
        default_factory=lambda: _safe_int(os.getenv("EMBEDDING_DIMENSION", "384"), 384)
    )
    batch_size: int = Field(
        default_factory=lambda: _safe_int(os.getenv("EMBEDDING_BATCH_SIZE", "32"), 32)
    )
    ollama_host: str = Field(
        default_factory=lambda: os.getenv("OLLAMA_HOST", "http://localhost:11434")
    )
    timeout: int = Field(
        default_factory=lambda: _safe_int(os.getenv("OLLAMA_TIMEOUT", "30"), 30)
    )
    cache_embeddings: bool = True
    max_concurrent_requests: int = Field(
        default_factory=lambda: _safe_int(os.getenv("EMBEDDING_CONCURRENCY", "5"), 5)
    )
...
class RedisConfig(BaseModel):
    ...
    port: int = Field(
        default_factory=lambda: _safe_int(os.getenv("REDIS_PORT", "6379"), 6379)
    )
    db: int = 0
    ttl: int = 3600
...
class AuthConfig(BaseModel):
    ...
    token_expiry_hours: int = Field(
        default_factory=lambda: _safe_int(os.getenv("JWT_EXPIRY_HOURS", "24"), 24)
    )
    enable_registration: bool = Field(
        default_factory=lambda: os.getenv("ENABLE_REGISTRATION", "false").lower()
        == "true"
    )
...
class ApiConfig(BaseModel):
    ...
    rate_limit_per_minute: int = Field(
        default_factory=lambda: _safe_int(os.getenv("RATE_LIMIT_PER_MINUTE", "60"), 60)
    )
    max_file_size_mb: int = Field(
        default_factory=lambda: _safe_int(os.getenv("MAX_FILE_SIZE_MB", "100"), 100)
    )
    ...
    request_timeout: int = Field(
        default_factory=lambda: _safe_int(os.getenv("REQUEST_TIMEOUT", "300"), 300)
    )
