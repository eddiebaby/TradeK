    ...
    def _validate_file_path(cls, file_path: str) -> Path:
        ...
        try:
            ...
            if any(pattern in file_path for pattern in suspicious_patterns):
                raise ValueError(
                    f"File path contains suspicious characters: {file_path}"
                )
...
class Chunk(BaseModel):
    ...
    embedding_id: str | None = Field(default=None, description="ID in vector DB")

    # Location information
    chapter: str | None = Field(
        default=None, description="Chapter title if available"
    )
    section: str | None = Field(
        default=None, description="Section title if available"
    )
...
class SearchResult(BaseModel):
    ...
    context_before: str | None = Field(default=None)
    context_after: str | None = Field(default=None)
...
class SearchResponse(BaseModel):
    ...
    interpreted_query: dict[str, Any] | None = Field(default=None)
