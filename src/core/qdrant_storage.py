...
class QdrantStorage(VectorStorageInterface):
    ...
    def __init__(self, collection_name: str | None = None):
        ...
        logger.info(
            f"Initialized Qdrant storage with collection: {self.collection_name}"
        )

    def _build_advanced_filter(self, filter_dict: dict[str, Any]) -> Filter | None:
        ...
        if "book_ids" in filter_dict and filter_dict["book_ids"]:
            ...
            if len(book_ids) == 1:
                conditions.append(
                    FieldCondition(key="book_id", match=MatchValue(value=book_ids[0]))
                )
            else:
                conditions.append(
                    FieldCondition(key="book_id", match=MatchAny(any=book_ids))
                )

        if "chunk_type" in filter_dict:
            conditions.append(
                FieldCondition(
                    key="chunk_type", match=MatchValue(value=filter_dict["chunk_type"])
                )
            )
        ...
        if "created_after" in filter_dict:
            try:
                ...
                conditions.append(
                    FieldCondition(
                        key="created_at", range=DatetimeRange(gte=after_date)
                    )
                )
            except (ValueError, TypeError) as e:
        ...
        if "created_before" in filter_dict:
            try:
                ...
                conditions.append(
                    FieldCondition(
                        key="created_at", range=DatetimeRange(lte=before_date)
                    )
                )
            except (ValueError, TypeError) as e:
        ...
        if "pages" in filter_dict and filter_dict["pages"]:
            try:
                ...
                conditions.append(
                    FieldCondition(key="page_start", match=MatchAny(any=page_numbers))
                )
            except (ValueError, TypeError) as e:
        ...
        if "chapters" in filter_dict and filter_dict["chapters"]:
            ...
            if len(chapters) == 1:
                conditions.append(
                    FieldCondition(key="chapter", match=MatchValue(value=chapters[0]))
                )
            else:
                conditions.append(
                    FieldCondition(key="chapter", match=MatchAny(any=chapters))
                )
        ...
        if "sections" in filter_dict and filter_dict["sections"]:
            ...
            if len(sections) == 1:
                conditions.append(
                    FieldCondition(key="section", match=MatchValue(value=sections[0]))
                )
            else:
                conditions.append(
        ...
        if "section_pattern" in filter_dict:
            ...
            conditions.append(
                FieldCondition(
                    key="section", match=MatchText(text=filter_dict["section_pattern"])
                )
            )
        ...
        if "exclude_books" in filter_dict and filter_dict["exclude_books"]:
            ...
            if len(exclude_books) == 1:
                must_not_conditions.append(
                    FieldCondition(
                        key="book_id", match=MatchValue(value=exclude_books[0])
                    )
                )
            else:
                must_not_conditions.append(
                    FieldCondition(key="book_id", match=MatchAny(any=exclude_books))
                )
        ...
        if "exclude_chapters" in filter_dict and filter_dict["exclude_chapters"]:
            ...
            if len(exclude_chapters) == 1:
                must_not_conditions.append(
                    FieldCondition(
                        key="chapter", match=MatchValue(value=exclude_chapters[0])
                    )
                )
            else:
                must_not_conditions.append(
                    FieldCondition(key="chapter", match=MatchAny(any=exclude_chapters))
                )
